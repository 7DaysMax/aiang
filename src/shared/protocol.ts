...
export type ClientCommand =
  ...
  | { type: "fs.findFolderByName"; name: string }
  ...
  /** 拉取 DeepSeek 账户余额（底部栏 DP 余额指标）。 */
  | { type: "deepseek.getBalance" }
  /** 设置页「检测连接」：验证 key 并拉取模型列表。 */
  | { type: "deepseek.testConnection" }
  /** 设置页「识图服务」检测连接：用 1×1 PNG 验证视觉模型可用。 */
  | {
  ...
  /** 设置页「热门技能」：分类返回编程/逆向各前 10 个（服务端聚合 + 缓存）。 */
  | { type: "skills.hot" }
