...
interface RightSidebarState {
  ...
  goBackBrowser: (projectId: string) => void
  goForwardBrowser: (projectId: string) => void
...
export const useRightSidebarStore = create<RightSidebarState>()(
  persist(
    (set) => ({
      ...
      goBackBrowser: (projectId) => set((state) => {
        const current = getProjectBrowserState(state.projectBrowser, projectId)
        if (current.historyIndex <= 0) return state
        ...
        return {
          projectBrowser: {
            ...
            [projectId]: {
              ...current,
              address: current.history[historyIndex] ?? current.address,
              historyIndex,
            },
          },
        }
      }),
      goForwardBrowser: (projectId) => set((state) => {
        const current = getProjectBrowserState(state.projectBrowser, projectId)
        if (current.historyIndex >= current.history.length - 1) return state
        const historyIndex = current.historyIndex + 1
        return {
          projectBrowser: {
            ...state.projectBrowser,
