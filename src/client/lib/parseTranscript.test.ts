...
describe("processTranscriptMessages", () => {
  test("hydrates thinking entries as assistant_thinking", () => {
    const messages = processTranscriptMessages([
      entry({ kind: "thinking", text: "Reasoning about the puzzle." }),
    ])
    ...
    expect(messages[0].text).toBe("Reasoning about the puzzle.")
  })

  test("merges streaming partial entries that share a messageId", () => {
    const messages = processTranscriptMessages([
      entry({ kind: "thinking", messageId: "step-1", text: "先分析" }),
      entry({ kind: "thinking", messageId: "step-1", text: "需求，再动手" }),
      entry({ kind: "assistant_text", messageId: "step-1", text: "好的，" }),
      entry({ kind: "assistant_text", messageId: "step-1", text: "我来改" }),
      entry({ kind: "thinking", messageId: "step-2", text: "另一个步骤" }),
    ])

    expect(messages).toHaveLength(3)
    expect(messages[0]?.kind).toBe("assistant_thinking")
    if (messages[0]?.kind !== "assistant_thinking") throw new Error("unexpected message")
    expect(messages[0].text).toBe("先分析需求，再动手")
    expect(messages[1]?.kind).toBe("assistant_text")
    if (messages[1]?.kind !== "assistant_text") throw new Error("unexpected message")
    expect(messages[1].text).toBe("好的，我来改")
    expect(messages[2]?.kind).toBe("assistant_thinking")
    if (messages[2]?.kind !== "assistant_thinking") throw new Error("unexpected message")
  ...
  test("keeps separate messages when messageIds differ", () => {
    const messages = processTranscriptMessages([
      ...
      entry({ kind: "assistant_text", messageId: "b", text: "two" }),
      entry({ kind: "assistant_text", text: "three" }),
    ])

    expect(messages).toHaveLength(3)
    expect(messages.map((message) => (message.kind === "assistant_text" ? message.text : ""))).toEqual(["one", "two", "three"])
  })
  ...
  test("stamps resultTimestamp so the activity feed can show per-tool duration", () => {
    const start = 1_700_000_000_000
    const finish = start + 3_250
    const messages = processTranscriptMessages([
      {
        ...entry({
          kind: "tool_call",
    ...
    expect(messages).toHaveLength(1)
    if (messages[0]?.kind !== "tool") throw new Error("unexpected message")
    expect(messages[0].resultEntryId).toBeDefined()
    expect(messages[0].resultTimestamp).toBe(new Date(finish).toISOString())
    const durationMs = Date.parse(messages[0].resultTimestamp!) - Date.parse(messages[0].timestamp)
    expect(durationMs).toBe(3_250)
  })
