...
describe("formatSidebarAgeLabel", () => {
  ...
  test("clamps future timestamps to now", () => {
    expect(formatSidebarAgeLabel(now + 1_000, now)).toBe("刚刚")
  })

  test("shows now for sub-minute deltas", () => {
    expect(formatSidebarAgeLabel(now, now)).toBe("刚刚")
    expect(formatSidebarAgeLabel(now - 59_000, now)).toBe("刚刚")
  })

  test("shows whole minutes for under an hour", () => {
    expect(formatSidebarAgeLabel(now - 60_000, now)).toBe("1分钟")
    expect(formatSidebarAgeLabel(now - 61_000, now)).toBe("1分钟")
    expect(formatSidebarAgeLabel(now - 30 * 60_000, now)).toBe("30分钟")
  })

  test("shows whole hours for under a day", () => {
    expect(formatSidebarAgeLabel(now - 60 * 60_000, now)).toBe("1小时")
    expect(formatSidebarAgeLabel(now - 16 * 60 * 60_000, now)).toBe("16小时")
  })

  test("shows whole days for under a week", () => {
    expect(formatSidebarAgeLabel(now - 24 * 60 * 60_000, now)).toBe("1天")
    expect(formatSidebarAgeLabel(now - 6 * 24 * 60 * 60_000, now)).toBe("6天")
  })

  test("shows whole weeks for seven days and up", () => {
    expect(formatSidebarAgeLabel(now - 7 * 24 * 60 * 60_000, now)).toBe("1周")
    expect(formatSidebarAgeLabel(now - 14 * 24 * 60 * 60_000, now)).toBe("2周")
  })
})
