...
function buildTranscriptMessageRenderStates(
  ...
  // 每条消息生效的模型身份：最近一次 system_init 的 provider + model，
  // 用于 AI 回复顶部的图标 + 模型名。
  const modelsAt = new Array<AssistantModelIdentity | undefined>(messages.length)
  let currentModel: AssistantModelIdentity | undefined
  for (let index = 0; index < messages.length; index++) {
    const message = messages[index]!
    if (message.kind === "system_init") {
      currentModel = { provider: message.provider, model: message.model }
    }
