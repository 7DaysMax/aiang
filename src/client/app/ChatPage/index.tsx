...
import { deriveChatDockMetrics, deriveLatestContextWindowSnapshot } from "../../lib/contextWindow"
...
import { VirtualCommandDialog } from "./VirtualCommandDialog"
...
export function ChatPage() {
  ...
  const [virtualCommandDialog, setVirtualCommandDialog] = useState<{ mode: "commands" | "usage" } | null>(null)
  ...
  const dockMetrics = useMemo(
    () => deriveChatDockMetrics(state.chatSnapshot?.messages ?? []),
    [state.chatSnapshot?.messages],
  )
  ...
  const handleDiscardAllChanges = useCallback(() => {
    ...
    const files = state.chatDiffSnapshot?.files ?? []
    if (!chatId || files.length === 0) return

    void (async () => {
      const confirmed = await dialog.confirm({
        title: "Discard All Changes",
        description: `Discard changes in ${files.length} file${files.length === 1 ? "" : "s"}? This cannot be undone.`,
      ...
      const results = await Promise.allSettled(
        files.map((file) => state.socket.command({
      ...
      if (failures.length > 0) {
        ...
        await dialog.alert({
          ...
          description: `${failures.length} file${failures.length === 1 ? "" : "s"} could not be discarded: ${
  ...
  }, [dialog, state.activeChatId, state.chatDiffSnapshot?.files, state.socket])
  ...
  const handleToggleInsightsPanel = useCallback(() => {
    if (!projectId) return
    toggleRightPanel(projectId, "insights")
  }, [projectId, toggleRightPanel])
  ...
  const handleVirtualCommand = useCallback((command: VirtualCommand) => {
    switch (command.name) {
      // "model" never reaches ChatPage: ChatInput opens the composer's picker.
      case "clear":
        if (projectId) {
      ...
      case "config":
        ...
        return
      case "help":
        setVirtualCommandDialog({ mode: "commands" })
        return
      case "cost":
      case "status":
        setVirtualCommandDialog({ mode: "usage" })
        return
    }
  }, [navigate, projectId, state.handleCreateChat])
    ...
    >
      <CardContent className="flex flex-1 min-h-0 flex-col overflow-hidden p-0 relative">
        <ChatNavbar
          ...
          onToggleInsightsPanel={projectId ? handleToggleInsightsPanel : undefined}
        ...
        <ChatTranscriptViewport
          ...
          changesFiles={state.chatDiffSnapshot?.files ?? EMPTY_DIFF_SNAPSHOT.files}
          onOpenChangedFile={handleOpenDiffFile}
          onDiscardChangedFile={handleDiscardDiffFile}
          onDiscardAllChanges={handleDiscardAllChanges}
          onOpenChangesPanel={handleToggleGitPanel}
      ...
      />
      <VirtualCommandDialog
        open={virtualCommandDialog !== null}
        onOpenChange={(open) => {
          if (!open) setVirtualCommandDialog(null)
        }}
        mode={virtualCommandDialog?.mode ?? "commands"}
        metrics={dockMetrics}
        balance={balance}
  ...
  const rightPanelInner = activeRightPanel === "browser" && projectId
    ...
    : activeRightPanel === "files" && projectId
      ...
      : activeRightPanel === "insights" && projectId
        ? (
          <InsightsPanel
            entries={state.chatSnapshot?.messages ?? []}
            contextWindowSnapshot={contextWindowSnapshot}
            onClose={handleCloseRightSidebar}
          />
        )
