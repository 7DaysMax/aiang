...
/**
 * Paths the agent wrote/edited/deleted after the latest user prompt.
 * Used to scope the changes summary to "this turn" instead of the whole
 * uncommitted project tree (which only grows as the session goes on).
 */
export function collectLastTurnChangedPaths(rows: ResolvedTranscriptRow[]): Set<string> {
  let lastUserIndex = -1
  ...
  const takePath = (value: unknown) => {
    if (typeof value === "string" && value.trim()) paths.add(value)
  }

  for (let i = lastUserIndex + 1; i < rows.length; i++) {
    const row = rows[i]
    const tools = row.kind === "tool-group"
      ? row.messages.filter((m): m is Extract<typeof m, { kind: "tool" }> => m.kind === "tool")
      : row.kind === "single" && row.message.kind === "tool"
        ? [row.message]
        : []

    for (const tool of tools) {
      if (tool.toolKind === "write_file" || tool.toolKind === "edit_file" || tool.toolKind === "delete_file") {
        takePath((tool.input as { filePath?: string }).filePath)
      }
    }
  }
  return paths
...
interface ChatTranscriptViewportProps {
  ...
  /**
   * 回合结束后的改动汇总卡（对话框内展示的文件变更 +X −Y）。
   * 只在 files 非空且不在处理中时渲染。
   */
  changesFiles?: ChatDiffFile[]
  onOpenChangedFile?: (path: string) => void
  onDiscardChangedFile?: (path: string) => void
...
const TranscriptScrollerBody = memo(function TranscriptScrollerBody({
  ...
  // Scope the bottom card to files this turn actually wrote/edited — not the
  // whole uncommitted tree, which keeps stacking (+1000, +1200…) for the whole session.
  const lastTurnChangedPaths = useMemo(
    () => collectLastTurnChangedPaths(resolvedRows),
    [resolvedRows],
  )
  const turnScopedChangeFiles = useMemo(() => {
    if (!changesFiles || changesFiles.length === 0) return [] as ChatDiffFile[]
    if (lastTurnChangedPaths.size === 0) return [] as ChatDiffFile[]
    const scoped = changesFiles.filter((file) => lastTurnChangedPaths.has(file.path))
    // Prefer exact path matches; also allow basename match when tools used absolute paths
    // and the diff store reports relative (or vice versa).
    if (scoped.length > 0) return scoped
    const basenames = new Set(
      [...lastTurnChangedPaths].map((path) => path.split("/").pop()).filter(Boolean) as string[],
    )
    return changesFiles.filter((file) => basenames.has(file.path.split("/").pop() ?? ""))
  }, [changesFiles, lastTurnChangedPaths])

  const changesSummary = turnScopedChangeFiles.length > 0 && !isProcessing ? (
    <div className="mx-auto w-full max-w-[816px] px-2 pb-1">
      <ChangesSummaryCard
        files={turnScopedChangeFiles}
        actions={{
          onOpenFile: (path) => onOpenChangedFile?.(path),
          onDiscardFile: (path) => onDiscardChangedFile?.(path),
          onDiscardAll: () => onDiscardAllChanges?.(),
          onReview: () => onOpenChangesPanel?.(),
        }}
      />
    </div>
  ) : null
