...
import { CLOUD_SERVICE_NAME } from "../../shared/branding"
      ...
      <MachineSection>
        {pairedOrigin ? (
          ...
          <button type="button" onClick={() => setPairDialogOpen(true)} className={SIDEBAR_BUTTON_CLASS}>
            <span className="flex min-w-0 items-center gap-2">
              <Cloud className="ml-[1px] size-4 shrink-0" />
              <span className="truncate text-xs font-medium">设置 {CLOUD_SERVICE_NAME}</span>
            </span>
            <ExternalLink className="size-3.5 shrink-0 opacity-60" />
          </button>
        )}
        <Dialog open={pairDialogOpen} onOpenChange={setPairDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>随时随地使用这台机器</DialogTitle>
              <DialogDescription>
                获得一个可在任意浏览器访问的个人 URL，完全免费。
              </DialogDescription>
            </DialogHeader>
            <DialogBody>
            ...
            {machines.map((machine) => {
              ...
              return (
                <PopoverMenuItem
                  ...
                  label={machine.name}
                  description={`${machine.subdomain}.kanna.sh${machine.online ? "" : " · 离线"}`}
                />
              )
            })}
            <PopoverMenuItem
              ...
              label="管理机器"
              description={`在 ${CLOUD_SERVICE_NAME} 上添加或移除机器`}
            />
          </>
        )}
      </InputPopover>
