...
Please check the latest error first.`,
  ...
  test("renders assistant replies with model header, thinking card, and answer", () => {
    const html = renderTranscript([
      ...
      },
      {
        ...
        text: "Let me reason about the ropes.",
        timestamp: new Date().toISOString(),
      },
      {
        id: "text-1",
  ...
  test("treats counter-suffixed DeepSeek messageIds as the same reply", () => {
    const html = renderTranscript([
      ...
      {
        id: "thinking-1",
        messageId: "d85426cb-0ada-410e-b209-000000000000",
        kind: "assistant_thinking",
        text: "Reasoning…",
        timestamp: new Date().toISOString(),
      },
      {
        id: "text-1",
        messageId: "d85426cb-0ada-410e-b209-000000000001",
        kind: "assistant_text",
        text: "The answer is Wednesday.",
        timestamp: new Date().toISOString(),
      },
    ])
  ...
  test("shows a thinking spinner while the thinking entry is still the latest row", () => {
    const html = renderTranscript([
      {
        id: "system-1",
        kind: "system_init",
  ...
  test("shows the model header on assistant text without a thinking prefix", () => {
    const html = renderTranscript([
      ...
      },
      {
        id: "text-1",
        kind: "assistant_text",
        text: "Plain answer without thinking.",
        timestamp: new Date().toISOString(),
      },
    ])
  ...
  test("labels the session init after a handoff with only the destination harness", () => {
    ...
    expect(html).not.toContain("模型已更改")
  })
