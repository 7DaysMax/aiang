...
export function useKannaState(activeChatId: string | null): KannaState {
  ...
  const resolveProjectIdForStartChat = useCallback(async (intent: StartChatIntent): Promise<{ projectId: string; localPath?: string }> => {
    ...
    // 拖入等场景允许自定义显示名：打开后立即写入 sidebarTitle，侧边栏
    // 固定显示这个名字（不被 repo/branch 覆盖）。
    if (intent.project.mode === "existing" && intent.project.sidebarTitle) {
      await socket.command({ type: "project.rename", projectId: result.projectId, title: intent.project.sidebarTitle })
    }
