...
/** "/Users/jake/.claude/skills/foo/SKILL.md" → "~/.claude/skills/foo"（含 Aiang 的 ~/.aiang 目录）。 */
function formatSkillLocation(skillPath: string) {
  return skillPath
    .replace(/\/SKILL\.md$/, "")
    .replace(/^.*?(?=\/\.(?:agents|claude|cursor|codex|aiang)\/)/, "~")
}
...
export function SkillsSection({
  ...
  const [hotSkills, setHotSkills] = useState<HotSkillsSnapshot | null>(null)
  const [hotCategory, setHotCategory] = useState<"programming" | "reverse">("programming")
  ...
  async function loadHotSkills() {
    ...
    setHotError(null)
    try {
      const snapshot = await socket.command<HotSkillsSnapshot>({ type: "skills.hot" })
      setHotSkills(snapshot)
    } catch (error) {
      setHotError(error instanceof Error ? error.message : "Unable to load hot skills.")
    } finally {
      setHotLoading(false)
    }
  }
  ...
  const hotCategorySkills = hotSkills?.categories.find((category) => category.id === hotCategory)?.skills
    ...
    <div className="flex flex-col gap-6">
        ...
        {query.trim().length < 2 ? (
          <div className="flex flex-col gap-3">
            <SegmentedControl
              ...
              className="w-fit"
            />
            {hotLoading && !hotSkills ? (
              <div className="flex items-center gap-2 rounded-lg border border-border bg-card/30 p-3 text-sm text-muted-foreground">
            ...
            {hotCategorySkills && hotCategorySkills.length > 0 ? (
              <div className="grid gap-3 md:grid-cols-2">
                {hotCategorySkills.map((skill) => (
                  <SkillResultCard
                    key={skill.id}
                    skill={skill}
                    installing={installingSkillId === skill.id}
      ...
      <section className="flex flex-col gap-3">
        ...
        {installedSkills.length > 0 ? (
          ...
          <div className="rounded-lg border border-border bg-card/30 p-3 text-sm text-muted-foreground">
            还没有已安装的技能。在上方「编程 / 逆向」热门分类里挑一个，或搜索 skills.sh 上的技能，点击 Get 安装后就会显示在这里。
          </div>
        ) : null}
      </section>
    </div>
