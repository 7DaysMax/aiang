...
import { DEFAULT_NEW_PROJECTS_DIRECTORY, type AppSettingsSnapshot } from "../../../shared/types"
...
export function GeneralSection({
  ...
  const dockMetrics = appSettings?.dockMetrics ?? {
    balance: true,
    cacheHitRate: true,
  ...
  async function handleDockMetricChange(
    key: keyof AppSettingsSnapshot["dockMetrics"],
    nextValue: "enabled" | "disabled",
  ) {
    try {
      setAppSettingsError(null)
      await handleWriteAppSettings({ dockMetrics: { [key]: nextValue === "enabled" } })
    } catch (caught) {
      setAppSettingsError(caught instanceof Error ? caught.message : "Unable to save dock metrics settings.")
    }
  }
      ...
      <div className="border-b border-border">
        ...
        {editorPreset === "custom" ? (
          <div className="border-t border-border">
            <div className="flex flex-wrap justify-between gap-8 py-5 pl-6">
              <div className="min-w-0 max-w-xl grow basis-72">
                <div className="text-sm font-medium text-foreground">Command Template</div>
                <div className="mt-1 break-words text-[13px] text-muted-foreground">
                  Include {"{path}"} and optionally {"{line}"} and {"{column}"} in your command.
                </div>
              </div>
              <div className="flex w-full min-w-0 max-w-[420px] flex-col items-stretch gap-2 md:w-auto md:shrink-0 md:max-w-[calc(100%-1.5rem)]">
                <Input
                  type="text"
        ...
        <SettingsRow def={SETTINGS_ROWS.dockMetrics} alignStart>
          <div className="flex w-full min-w-0 flex-col gap-3">
            <div className="flex items-center justify-between gap-8">
              <div className="text-[13px] text-muted-foreground">DP 余额</div>
              <SegmentedControl
                value={dockMetrics.balance ? "enabled" : "disabled"}
                onValueChange={(value) => {
                  void handleDockMetricChange("balance", value)
                }}
            ...
            <div className="flex items-center justify-between gap-8">
              <div className="text-[13px] text-muted-foreground">本次缓存命中率</div>
              <SegmentedControl
                value={dockMetrics.cacheHitRate ? "enabled" : "disabled"}
                onValueChange={(value) => {
                  void handleDockMetricChange("cacheHitRate", value)
                }}
                options={ENABLED_DISABLED_OPTIONS}
                size="sm"
              />
            ...
            <div className="flex items-center justify-between gap-8">
              <div className="text-[13px] text-muted-foreground">平均缓存命中率</div>
              <SegmentedControl
                value={dockMetrics.averageCacheHitRate ? "enabled" : "disabled"}
                onValueChange={(value) => {
                  void handleDockMetricChange("averageCacheHitRate", value)
                }}
                options={ENABLED_DISABLED_OPTIONS}
                size="sm"
              />
            ...
            <div className="flex items-center justify-between gap-8">
              <div className="text-[13px] text-muted-foreground">会话 tokens</div>
              <SegmentedControl
                value={dockMetrics.sessionTokens ? "enabled" : "disabled"}
                onValueChange={(value) => {
                  void handleDockMetricChange("sessionTokens", value)
                }}
                options={ENABLED_DISABLED_OPTIONS}
                size="sm"
              />
            ...
            <div className="flex items-center justify-between gap-8">
              <div className="text-[13px] text-muted-foreground">DeepSeek 服务状态</div>
              <SegmentedControl
                value={dockMetrics.serviceStatus ? "enabled" : "disabled"}
                onValueChange={(value) => {
                  void handleDockMetricChange("serviceStatus", value)
                }}
                options={ENABLED_DISABLED_OPTIONS}
                size="sm"
              />
      ...
      <Dialog open={analyticsDialogOpen} onOpenChange={setAnalyticsDialogOpen}>
        <DialogContent size="lg">
          <DialogBody className="space-y-4">
            <DialogTitle>追踪事件</DialogTitle>
            <div className="text-sm text-muted-foreground">
              Aiang 会上报以下事件名，以及下方按事件类型区分的少量属性键。
            </div>
