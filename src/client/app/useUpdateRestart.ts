...
export function useUpdateRestart(params: {
  ...
  const handleInstallNightly = useCallback(async () => {
    const confirmed = await dialog.confirm({
      title: "更新到每日构建？",
      description: "Aiang 将从 GitHub 下载 main 分支的最新代码，在本机从源码构建并重启。这需要几分钟，每日构建可能不如正式版稳定；下一个正式版发布后会自动回到稳定版。",
      confirmLabel: "构建每日版",
    })
    if (!confirmed) return
    await runInstallCommand(
  ...
  const handleInstallStable = useCallback(async () => {
    const confirmed = await dialog.confirm({
      title: "回到稳定版？",
      description: "Aiang 将重新安装最新的正式版并重启。",
      confirmLabel: "重装稳定版",
    })
    if (!confirmed) return
    await runInstallCommand(
