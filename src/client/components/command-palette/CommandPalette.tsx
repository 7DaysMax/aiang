...
import { PROVIDER_ENGINE_LABELS, PROVIDER_ICONS } from "../chat-ui/ChatPreferenceControls"
...
const ADD_PROJECT_STATIC_ACTIONS = [
  { id: "clone-github", title: "从 GitHub 克隆…", keywords: ["repo", "git", "clone", "checkout", "克隆"] },
  { id: "create-new", title: "新建…", keywords: ["blank", "empty", "init", "new project", "start", "新建项目"] },
  { id: "choose-existing", title: "选择已有…", keywords: ["browse", "folder", "open", "filesystem", "directory", "path", "添加已有"] },
] as const
...
export function CommandPalette({ state }: { state: KannaState }) {
  ...
  const actions = useMemo<PaletteAction[]>(() => {
    ...
    if (projectId) {
      list.push({
        ...
        title: currentProjectTitle ? `在 ${currentProjectTitle} 中新建对话` : "在当前项目中新建对话…",
      ...
      list.push({
        id: "project-chats",
        title: currentProjectTitle ? `${currentProjectTitle} 中的对话` : "当前项目中的对话…",
    ...
    list.push({
      ...
      title: nextTheme === "dark" ? "深色模式" : "浅色模式",
    ...
    list.push({
      ...
      title: recentChatsInSidebarOn ? "关闭新侧边栏" : "开启新侧边栏",
    ...
    if (onChatPage && projectId) {
      ...
      list.push({
        id: "git-panel",
        title: gitPanelVisible ? "隐藏 Git 面板" : "显示 Git 面板",
      ...
      list.push({
        id: "browser-panel",
        title: browserPanelVisible ? "隐藏浏览器面板" : "显示浏览器面板",
    ...
    if (state.activeChatId) {
      ...
      if (currentChatGroup) {
        list.push({
          id: "hide-project",
          title: `隐藏 ${currentChatGroup.title}`,
    ...
    if (onChatPage) {
      ...
      for (const model of composer.models) {
        ...
        list.push({
          ...
          icon: <Box className={ICON_CLASS} />,
          hint: isCurrent ? "当前模型" : `${providerLabel} 模型`,
          searchOnly: true,
          run: () => {
            close()
  ...
  const inputPlaceholder = page === "models"
    ? `搜索 ${composer.providerConfig?.label ?? "模型服务"} 模型…`
    : page === "harness"
      ? "选择引擎…"
      : page === "new-thread"
        ? "选择项目…"
        ...
        >
          ...
          {page === "root" ? (() => {
            const reviewGroup = reviewResults.length > 0 ? (
              <CommandGroup key="review" heading="评审">
            ...
            const inProgressGroup = inProgressResults.length > 0 ? (
              <CommandGroup key="in-progress" heading="进行中">
            ...
            const threadsGroup = threadResults.length > 0 ? (
              <CommandGroup key="threads" heading={trimmedQuery ? "对话" : "最近"}>
            ...
            const actionsGroup = rankedActions.length > 0 ? (
              <CommandGroup key="actions" heading="操作">
            ...
            const projectsGroup = projectSearchResults.length > 0 ? (
              <CommandGroup key="projects" heading="项目">
            ...
            const allProjectsGroup = allProjectSearchResults.length > 0 ? (
              <CommandGroup key="all-projects" heading="所有项目">
          ...
          {page === "models" ? (
            <CommandGroup heading={`${composer.providerConfig?.label ?? "模型服务"} 模型`}>
              ...
              {harnessResults.map((provider) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="truncate text-xs text-muted-foreground">{PROVIDER_ENGINE_LABELS[provider.id]}</span>
          ...
          {page === "project-chats" ? (
            <>
              ...
              [
                { key: "in-progress", label: "进行中", threads: projectChatSections.inProgress },
                { key: "review", label: "评审", threads: projectChatSections.review },
                ...projectChatSections.buckets.map((bucket) => ({ key: bucket.key, label: bucket.label, threads: bucket.threads })),
                { key: "archived", label: "已归档", threads: projectChatSections.archived },
              ]
              ...
              <CommandGroup heading={projectChatsTitle ? `${projectChatsTitle} 中的对话` : "项目对话"}>
          ...
          {page === "add-project" ? (
            <>
              ...
              {addProjectActionRows.length > 0 ? (
                <CommandGroup key="add-actions" heading="添加项目">
                ...
                {dir && !browser.error ? (
                  <>
                    ...
                    {visibleBrowseEntries.length > 0 || browseCreateVisible || dir.truncated ? (
                      <CommandGroup key={`${dir.path}:entries`} heading="此文件夹中">
