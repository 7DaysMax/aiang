...
export function InputPopover({
  ...
  /** Controlled open state (e.g. /model opens the model picker). */
  open?: boolean
    ...
    <Popover open={openValue} onOpenChange={setOpenValue}>
      ...
      <PopoverContent align={align} className="w-64 overflow-hidden p-0">
        ...
        <div className="max-h-80 overflow-y-auto divide-y divide-border/60">
          {typeof children === "function" ? children(() => setOpenValue(false)) : children}
        </div>
      </PopoverContent>
    </Popover>
  )
...
export type ModelOptionChange =
  ...
  | { type: "deepseekReasoningEffort"; effort: DeepSeekReasoningEffort }
...
interface ChatPreferenceControlsProps {
  ...
  onEditModels?: () => void
  /** Controlled model-picker popover state (e.g. the /model command). */
  ...
  return (
    <div className={cn("flex md:justify-center items-center gap-0.5", className)}>
      ...
      >
        {(close) => (
          <ModelPickerList
            ...
            renderLabel={(candidate) => candidate.label}
      ...
      {controls.fastMode ? (() => {
        ...
        const fastLabel = selectedProvider === "cursor" ? "快速" : "快速模式"
        return (
          <InputPopover
            trigger={(
              <>
