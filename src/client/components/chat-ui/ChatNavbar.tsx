import { Check, Flower, FolderTree, GitBranch, Globe, LayoutDashboard, Loader2, Menu, MoreHorizontal, PanelLeft, PanelRight, Search, SquarePen, Terminal, UserRoundPlus } from "lucide-react"
...
interface Props {
  ...
  onToggleEmbeddedTerminal?: () => void
  rightPanel?: "hidden" | "insights" | "git" | "browser" | "files"
  ...
  repoUrl?: string
  hasGitRepo?: boolean
  gitStatus?: "unknown" | "ready" | "no_repo" | "snapshot"
}
...
export function ChatNavbar({
  ...
  const handleCloseRightPanel = rightPanel === "browser" ? onToggleBrowserPanel : rightPanel === "git" ? onToggleGitPanel : rightPanel === "files" ? onToggleFilesPanel : rightPanel === "insights" ? onToggleInsightsPanel : undefined
  const showInsightsPanelButton = rightPanel === "hidden" || rightPanel !== "insights"
  const showBrowserPanelButton = rightPanel === "hidden" || rightPanel !== "browser"
  const showGitPanelButton = rightPanel === "hidden" || rightPanel !== "git"
  const showFilesPanelButton = rightPanel === "hidden" || rightPanel !== "files"
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-center gap-2 w-full">
        ...
        {localPath && (onOpenExternal || onToggleEmbeddedTerminal || onToggleGitPanel || onToggleBrowserPanel || onToggleFilesPanel || onToggleInsightsPanel || onExportTranscript) ? (
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {(onToggleEmbeddedTerminal || onToggleGitPanel || onToggleBrowserPanel || onToggleFilesPanel || onToggleInsightsPanel || onExportTranscript) ? (
              <div className="flex items-center  rounded-[9px] h-[30px]">
                ...
                {onToggleInsightsPanel && showInsightsPanelButton ? (
                  <Button
                    variant="ghost"
                    size="none"
                  ...
                  >
                    <LayoutDashboard strokeWidth={2.25} className="h-4 max-md:h-5 max-md:w-5" />
                  </Button>
                ) : null}
                {onToggleFilesPanel && showFilesPanelButton ? (
                  ...
                  >
                    <FolderTree strokeWidth={2.25} className="h-4 max-md:h-5 max-md:w-5" />
                  </Button>
                ) : null}
