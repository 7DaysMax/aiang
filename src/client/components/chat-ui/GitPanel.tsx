...
function GitPanelImpl({
  ...
  // 非 git 项目走本地快照跟踪：没有分支/历史/提交，只展示改动与撤销。
  const isSnapshotMode = diffs.status === "snapshot"
  const effectiveViewMode = isSnapshotMode ? "changes" : viewMode
  ...
  const diffFilesList = (
              <div className="pt-1.5 pb-10">
                ...
                {!isSnapshotMode && effectiveViewMode === "changes" ? (
          ...
          <div className="flex min-w-0 flex-1 items-center gap-2">
            {isSnapshotMode ? (
              <div className="flex min-w-0 items-center gap-2 text-xs text-muted-foreground">
                <span className="size-2 shrink-0 rounded-full bg-logo" aria-hidden="true" />
                <span className="truncate">本地快照模式（无需 git）</span>
              </div>
            ) : (
              <BranchSwitcher
