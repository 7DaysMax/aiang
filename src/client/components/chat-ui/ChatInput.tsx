...
/** 「优化提示词」失败时的中文提示。 */
const OPTIMIZE_PROMPT_ERROR_MESSAGE: Record<string, string> = {
  missing_key: "未配置 DeepSeek API Key，请先在「设置 → 模型服务」中填写。",
  invalid_key: "DeepSeek API Key 格式不正确，请检查设置。",
  unauthorized: "DeepSeek API Key 无效（401），请检查设置。",
  request_failed: "网络请求失败，请稍后重试。",
  empty_response: "模型没有返回内容，请重试。",
  empty_prompt: "请先输入要优化的内容。",
}
...
interface Props {
  ...
  /** Exact "/<name>" invocations of commands the harness filters out (e.g. /model, /clear). */
...
const ChatInputInner = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  const placeholder = projectLabel ? `在 ${projectLabel} 中构建` : "输入消息，开始构建…"
  ...
  const skillMenuItems = useMemo(
    () => {
      ...
      const skills = availableSkills ? filterSkillMenuItems(availableSkills, slashQuery) : []
      return mergeVirtualSkillMenuItems(skills, slashQuery)
    },
  ...
  async function handleOptimizePrompt() {
    const socket = useDeepSeekBalanceStore.getState().socket
    if (!socket || !hasTextToSend || optimizingPrompt) return

    setOptimizingPrompt(true)
    setOptimizeError(null)
    try {
      const result = await socket.command<DeepSeekPromptOptimizeResult>({
        type: "prompt.optimize",
        prompt: value,
      })
      if (result.ok && result.optimized) {
        setValue(result.optimized)
        if (chatId) setDraft(chatId, result.optimized)
        autoResize()
        requestAnimationFrame(() => {
          const element = textareaRef.current
          if (!element) return
        ...
        setOptimizeError(
          result.error
            ? (OPTIMIZE_PROMPT_ERROR_MESSAGE[result.error] ?? "优化失败，请稍后重试。")
            : "优化失败，请稍后重试。"
        )
      }
    } catch {
      setOptimizeError("优化失败，请稍后重试。")
    } finally {
  ...
  async function handleSubmit() {
    ...
    // Commands ccb's headless mode filters out are bridged to frontend
    // actions (model picker, new chat, settings…) instead of the harness.
    const virtualCommand = getVirtualCommand(nextValue)
    if (virtualCommand) {
      ...
      try {
        if (virtualCommand.name === "model") {
          // The composer's own picker (Claude Code /model behavior), not the
          // Default Models editor — the latter stays behind "添加模型…".
          setModelPickerOpen(true)
        } else {
          await onVirtualCommand?.(virtualCommand)
        }
        previousAttachments.forEach(cleanupAttachmentPreview)
      } catch (error) {
        console.error("[ChatInput] Virtual command failed:", error)
        setValue(nextValue)
        if (chatId) setDraft(chatId, nextValue)
  ...
  function handleKeyDown(event: React.KeyboardEvent) {
    ...
    // 回车发送（Shift+Enter 换行）。中文输入法组合中的 Enter 只是确认候选词，
    // 用 isComposing 拦掉避免误发；触屏设备同样保持回车发送，桌面体验优先。
    if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing && !disabled && hasTextToSend && !hasPendingUploads) {
      event.preventDefault()
      void handleSubmit()
    }
  }
