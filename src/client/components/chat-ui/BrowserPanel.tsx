import { ChevronLeft, ChevronRight, CornerDownLeft, Ellipsis, ExternalLink, Globe, Home, Minus, Play, Plus, RefreshCw, SquareArrowOutUpRight, Trash2, Zap } from "lucide-react"
import { memo, useCallback, useEffect, useRef, useState, type FocusEvent, type FormEvent, type KeyboardEvent, type ReactNode } from "react"
import type { LocalHttpServerInfo, ProjectQuickAction } from "../../../shared/protocol"
import type { KannaSocket } from "../../app/socket"
import { BROWSER_PROXY_PATH, browserProxyUrl, isLocalHostname } from "../../lib/browserProxy"
import {
...
function BrowserPanelImpl({ projectId, socket, onRunQuickAction }: BrowserPanelProps) {
  ...
  const goBackBrowser = useRightSidebarStore((store) => store.goBackBrowser)
  const goForwardBrowser = useRightSidebarStore((store) => store.goForwardBrowser)
  ...
  const history = browserState?.history ?? []
  const historyIndex = browserState?.historyIndex ?? -1
  const canGoBack = historyIndex > 0
  const canGoForward = historyIndex >= 0 && historyIndex < history.length - 1
  const iframeSrc = browserProxyUrl(address) ?? address
  const isDirectLocalUrl = (() => {
    if (iframeSrc !== address) return false
    try {
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      const href = event.data?.__aiangBrowserProxy
      if (typeof href !== "string") return
      let proxyUrl: URL
      try {
        proxyUrl = new URL(href)
      } catch {
        return
      }
      if (proxyUrl.pathname !== BROWSER_PROXY_PATH) return
      const target = proxyUrl.searchParams.get("url")
      if (target && target !== address) {
        navigateBrowser(projectId, target)
      }
    }
    window.addEventListener("message", handleMessage)
    return () => window.removeEventListener("message", handleMessage)
  }, [address, navigateBrowser, projectId])
    ...
    <div className="h-full min-h-0 border-l border-border bg-background md:min-w-[370px]">
      <div className="flex h-full min-h-0 flex-col">
        <div className="flex shrink-0 items-center gap-1 border-b border-border px-2 py-2">
          ...
          <BrowserToolbarButton label="Back" disabled={!canGoBack} onClick={() => goBackBrowser(projectId)}>
            <ChevronLeft className="h-4 w-4" />
          </BrowserToolbarButton>
          <BrowserToolbarButton label="Forward" disabled={!canGoForward} onClick={() => goForwardBrowser(projectId)}>
