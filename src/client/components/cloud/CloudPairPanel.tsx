...
import { CLOUD_SERVICE_NAME, CLI_COMMAND } from "../../../shared/branding"
...
function ManualPairInstructions() {
  return (
    <ol className="list-decimal space-y-2 pl-5 text-sm">
      ...
      <li>
        在本机终端运行{" "}
        <code className="rounded bg-muted px-1.5 py-0.5 text-xs">bunx {CLI_COMMAND} pair &lt;code&gt;</code>
        。
      </li>
    </ol>
  )
  ...
  return (
    <div className="space-y-4 py-2 text-center">
      ...
      <p className="text-sm text-muted-foreground">
        这台机器已上线：<span className="font-medium text-foreground">{host}</span> —— 只要 Youmi
        在运行，它就一直可访问。
      </p>
      ...
      <div className="space-y-3 py-2 text-center">
        <p className="text-sm text-muted-foreground">
          {session.status === "expired"
            ? "该链接已过期。"
            : `无法连接 ${CLOUD_SERVICE_NAME}${session.error ? ` (${session.error})` : ""}。`}
        </p>
        <button
          type="button"
          onClick={onRetry}
