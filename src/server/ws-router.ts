...
import { fetchDeepSeekBalance, optimizePrompt, testDeepSeekConnection } from "./deepseek-agent"
import { fetchDeepSeekStatus } from "./deepseek-status"
import { testVisionConnection } from "./vision"
import { detectCodex, installCodex } from "./codex-install"
...
interface CreateWsRouterArgs {
  /** DeepSeek 余额查询（可注入用于测试）。 */
  fetchDeepSeekBalanceImpl?: typeof fetchDeepSeekBalance
  /** 设置页「检测连接」（可注入用于测试）。 */
  testDeepSeekConnectionImpl?: typeof testDeepSeekConnection
  /** 设置页「识图服务」检测连接（可注入用于测试）。 */
  testVisionConnectionImpl?: typeof testVisionConnection
  /** 对话框「优化提示词」（可注入用于测试）。 */
  optimizePromptImpl?: typeof optimizePrompt
  /** 设置页「服务状态」：拉取 DeepSeek 官方状态页（可注入用于测试）。 */
  fetchDeepSeekStatusImpl?: typeof fetchDeepSeekStatus
  /** Codex 引擎探测（可注入用于测试）。 */
  detectCodexImpl?: typeof detectCodex
...
export function createWsRouter({
  fetchDeepSeekBalanceImpl = fetchDeepSeekBalance,
  testDeepSeekConnectionImpl = testDeepSeekConnection,
  testVisionConnectionImpl = testVisionConnection,
  optimizePromptImpl = optimizePrompt,
  fetchDeepSeekStatusImpl = fetchDeepSeekStatus,
  detectCodexImpl = detectCodex,
  installCodexImpl = installCodex,
  store,
  diffStore,
  worktreeProbe,
  agent,
  ...
  async function handleCommand(ws: ServerWebSocket<ClientState>, message: Extract<ClientEnvelope, { type: "command" }>) {
    ...
    try {
      switch (command.type) {
        ...
        case "fs.findFolderByName": {
          const result = { matches: findFolderByName(command.name) }
          send(ws, { v: PROTOCOL_VERSION, type: "ack", id, result })
          return
        }
        ...
        case "deepseek.getBalance": {
          const snapshot = await fetchDeepSeekBalanceImpl()
          send(ws, { v: PROTOCOL_VERSION, type: "ack", id, result: snapshot })
          return
        }
        case "deepseek.testConnection": {
          const result = await testDeepSeekConnectionImpl()
          send(ws, { v: PROTOCOL_VERSION, type: "ack", id, result })
          return
        }
        case "vision.testConnection": {
          const result = await testVisionConnectionImpl({
            enabled: true,
            provider: command.provider,
            apiKey: command.apiKey,
            baseUrl: command.baseUrl ?? "",
        ...
        case "deepseek.status": {
          const result = await fetchDeepSeekStatusImpl(command.force === true)
          send(ws, { v: PROTOCOL_VERSION, type: "ack", id, result })
          return
        }
        case "codex.detect": {
          const result = await detectCodexImpl()
          send(ws, { v: PROTOCOL_VERSION, type: "ack", id, result })
          return
        }
        case "codex.install": {
        ...
        case "prompt.optimize": {
          const result = await optimizePromptImpl(command.prompt)
          send(ws, { v: PROTOCOL_VERSION, type: "ack", id, result })
          return
        }
        case "skills.search": {
        ...
        case "skills.hot": {
          const snapshot = await fetchHotSkills()
          send(ws, { v: PROTOCOL_VERSION, type: "ack", id, result: snapshot })
          return
        }
        case "skills.install": {
        ...
        case "plugin.uninstall": {
          await uninstallPlugin(command.pluginName)
          send(ws, { v: PROTOCOL_VERSION, type: "ack", id, result: { ok: true } })
          return
        }
        case "project.open": {
          await requireExistingDirectory(command.localPath)
