...
describe("buildHandoffContext", () => {
  ...
  test("tolerates error results with a missing result field (legacy transcripts)", () => {
    const context = build([
      userPrompt("do it"),
      // 老版 JSONL 里的 result 条目可能没有 result 字段。
      timestamped({ kind: "result", subtype: "error", isError: true, durationMs: 1 } as never),
    ])

    expect(context).not.toBeNull()
    expect(context!.text).toContain("--- turn ended with error ---")
  })
  ...
  test("session_restore reason swaps the preamble and renders restore boundaries", () => {
    ...
    expect(text).toContain("restored from Youmi's saved transcript")
