...
describe("provider catalog normalization", () => {
  ...
  test("clamps DeepSeek V4 models inside the Claude entry to deepseek rules", () => {
    // medium 是 V3 时代的档位，V4 只认 low/high/max —— 归一化回默认 high。
    expect(normalizeClaudeModelOptions("deepseek-v4-pro", {
      claude: { reasoningEffort: "medium", contextWindow: "1m", fastMode: true },
    })).toEqual({
      reasoningEffort: "high",
      contextWindow: "200k",
      fastMode: false,
    })
    expect(normalizeClaudeModelOptions("deepseek-v4-flash", undefined, "max")).toEqual({
      reasoningEffort: "max",
      contextWindow: "200k",
      fastMode: false,
    })
