...
import { DEEPSEEK_CONTEXT_WINDOW_TOKENS } from "./deepseek-agent"
import { buildVisionMcpServerSpec, VISION_MCP_SERVER_NAME } from "./vision"

/**
 * 识图：通过 codex `-c` 配置覆盖注入 MCP server（深合并，不碰用户的
 * config.toml）。值按 TOML 解析，路径用 JSON 转义即可。
 */
function buildCodexMcpConfigArgs(): string[] {
  const visionSpec = buildVisionMcpServerSpec()
  if (!visionSpec) return []
  return [
    "-c",
    `mcp_servers.${VISION_MCP_SERVER_NAME}.command=${JSON.stringify(visionSpec.command)}`,
    "-c",
    `mcp_servers.${VISION_MCP_SERVER_NAME}.args=${JSON.stringify(visionSpec.args)}`,
  ]
}
import {
...
function codexSystemInitEntry(model: string): TranscriptEntry {
  return timestamped({
    ...
    provider: "codex",
    model,
    // 与 codex app-server 实际转发/翻译的工具面保持一致（见 itemToToolCalls
    // 与 genericDynamicToolCall）：Read/Glob/Grep/WebFetch/Task 等都会以
    // 工具调用形式出现在 transcript 里，会话头应完整列出而不是只列核心几个。
    tools: [
      "Bash", "Read", "Write", "Edit", "Delete", "Glob", "Grep",
      "WebSearch", "WebFetch", "TodoWrite", "AskUserQuestion", "ExitPlanMode",
      "Task", "NotebookEdit", "Skill", "Notify", "CronCreate", "CronDelete",
      "CronList", "EnterPlanMode", "KillShell",
    ],
...
/** 从 codex reasoning item 的 content/summary 里提取纯文本。codex 的推理
 * 内容可能是字符串、{text}、{content:[...]} 或嵌套数组；OpenAI 兼容通道的
 * encrypted_content 是密文，跳过。 */
function extractReasoningText(item: ReasoningItem): string {
  ...
  const visit = (value: unknown): void => {
    ...
    if (value && typeof value === "object") {
      ...
      for (const key of ["text", "reasoning", "content", "summary"]) {
  ...
  visit(item.content.length ? item.content : item.summary)
  return parts.join("")
}
...
function platformCodexCandidates(home: string): string[] {
  if (process.platform === "win32") {
    return [
      // Windows：Aiang 一键安装 / 用户目录 / ChatGPT 桌面版常见位置。
      path.join(home, ".aiang", "bin", "codex.exe"),
  ...
  return [
    // macOS：ChatGPT/Codex 桌面版内置的 codex CLI；GUI 启动的进程 PATH 常不含它。
    ...
    // Aiang 一键安装落地的位置。
    path.join(home, ".aiang", "bin", "codex"),
  ]
}

/** 解析可用的 codex 可执行文件；找不到时回退到 PATH 里的 "codex" 让 spawn 报错。 */
export function resolveCodexBinary(home: string = homedir()): string {
  for (const candidate of platformCodexCandidates(home)) {
    try {
...
export class CodexAppServerManager {
  ...
  constructor(args: { spawnProcess?: SpawnCodexAppServer } = {}) {
    this.spawnProcess = args.spawnProcess ?? ((cwd) =>
      spawn(resolveCodexBinary(), ["app-server", ...buildCodexMcpConfigArgs()], {
        cwd,
    ...
    if (notification.item.type === "agentMessage") {
      ...
      if (text.trim()) {
        ...
        pendingTurn.queue.push({
          ...
          entry: timestamped({
            kind: "assistant_text",
            messageId: notification.item.id,
            text,
          }),
        })
      }
  ...
  private handleTokenUsageUpdated(
    ...
    // Codex 桌面端对 deepseek 模型上报的是其内置元数据的窗口（258.4k），
    // DeepSeek V4 实际上下文为 1M —— 与 ccb 通道同一个规则，直接覆盖。
    if (pendingTurn.model.startsWith("deepseek-")) {
      usage.maxTokens = DEEPSEEK_CONTEXT_WINDOW_TOKENS
    }
