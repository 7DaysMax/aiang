...
describe("DiffStore", () => {
  ...
  describe("symlinks", () => {
    ...
    test("reads a symlink's patch as its target path", async () => {
      ...
      const { patch } = await store.readPatch({ projectId: "project-1", projectPath: repoRoot, path: ".claude/skills/shadcn" })
    ...
    test("a symlink to a file diffs as the link, not the file it points at", async () => {
      ...
      expect(link?.additions).toBe(1)
      const { patch } = await store.readPatch({ projectId: "project-1", projectPath: repoRoot, path: "alias.txt" })
    ...
    test("a dangling symlink still counts — git commits it either way", async () => {
      ...
      expect(link?.additions).toBe(1)
      const { patch } = await store.readPatch({ projectId: "project-1", projectPath: repoRoot, path: "broken-link" })
      expect(patch).toContain("+nowhere/at/all")
    })
  })

  test("tracks a non-git folder via local snapshot baseline", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "kanna-no-repo-"))
    const dataDir = await mkdtemp(path.join(tmpdir(), "kanna-no-repo-data-"))
    ...
    expect(snapshot.branchName).toBeUndefined()
    expect(snapshot.files).toEqual([])
    expect(snapshot.branchHistory).toEqual({ entries: [] })
  })
  ...
  test("refreshSnapshot tolerates huge untracked files and readPatch refuses to load them", async () => {
    ...
    await expect(store.readPatch({ projectId: "project-1", projectPath: repoRoot, path: "artifact.data" }))
...
describe("DiffStore snapshot mode (no git)", () => {
  test("tracks adds/edits/deletes against a local baseline and reverts without git", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "kanna-snapshot-"))
    const dataDir = await mkdtemp(path.join(tmpdir(), "kanna-snapshot-data-"))
    tempDirs.push(root, dataDir)
    ...
    // 首次刷新：当前树固化为基线，无改动。
    await store.refreshSnapshot("project-1", root)
    let snapshot = store.getProjectSnapshot("project-1")
    expect(snapshot.status).toBe("snapshot")
    expect(snapshot.files).toHaveLength(0)

    // 修改 + 新增 + 删除。
    await writeFile(path.join(root, "app.ts"), "const a = 1\nconst b = 3\nconst c = 4\n", "utf8")
    await writeFile(path.join(root, "new-file.ts"), "export const x = 1\n", "utf8")
    await rm(path.join(root, "readme.md"))

    await store.refreshSnapshot("project-1", root)
    snapshot = store.getProjectSnapshot("project-1")
    expect(snapshot.status).toBe("snapshot")
    expect(snapshot.files).toHaveLength(3)
    const byPath = Object.fromEntries(snapshot.files.map((file) => [file.path, file]))
    expect(byPath["app.ts"]?.changeType).toBe("modified")
    expect(byPath["app.ts"]?.additions).toBe(2)
    expect(byPath["app.ts"]?.deletions).toBe(1)
    expect(byPath["new-file.ts"]?.changeType).toBe("added")
    expect(byPath["new-file.ts"]?.isUntracked).toBe(true)
    expect(byPath["readme.md"]?.changeType).toBe("deleted")

    // patch：纯 JS 生成 git 风格 unified diff，带 +/- 与语言可识别头。
    const { patch } = await store.readPatch({ projectId: "project-1", projectPath: root, path: "app.ts" })
    expect(patch).toContain("diff --git a/app.ts b/app.ts")
    expect(patch).toContain("-const b = 2")
    expect(patch).toContain("+const b = 3")
    expect(patch).toContain("+const c = 4")

    // 撤销修改：恢复基线内容。
    await store.discardFile({ projectId: "project-1", projectPath: root, path: "app.ts" })
    expect(await readFile(path.join(root, "app.ts"), "utf8")).toBe("const a = 1\nconst b = 2\n")
    // 撤销新增：删除文件。
    await store.discardFile({ projectId: "project-1", projectPath: root, path: "new-file.ts" })
    expect(await readFile(path.join(root, "new-file.ts")).catch(() => null)).toBeNull()
    // 撤销删除：恢复文件。
    await store.discardFile({ projectId: "project-1", projectPath: root, path: "readme.md" })
    expect(await readFile(path.join(root, "readme.md"), "utf8")).toBe("hi\n")
  ...
  test("ignore file stops tracking it in snapshot mode", async () => {
    ...
    tempDirs.push(root, dataDir)
    await writeFile(path.join(root, "ignored.ts"), "export const a = 1\n", "utf8")

    const store = new DiffStore(dataDir)
    await store.initialize()
    await store.refreshSnapshot("project-1", root)

    await writeFile(path.join(root, "ignored.ts"), "export const a = 2\n", "utf8")
    await store.refreshSnapshot("project-1", root)
    expect(store.getProjectSnapshot("project-1").files).toHaveLength(1)

    await store.ignoreFile({ projectId: "project-1", projectPath: root, path: "ignored.ts" })
    expect(store.getProjectSnapshot("project-1").files).toHaveLength(0)
