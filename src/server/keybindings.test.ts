...
describe("KeybindingsManager", () => {
  ...
  test("uses the runtime profile for the default keybindings path", () => {
    const previous = process.env.AIANG_RUNTIME_PROFILE
    process.env.AIANG_RUNTIME_PROFILE = "dev"

    const manager = new KeybindingsManager()

    expect(manager.filePath).toEndWith("/.aiang-dev/keybindings.json")

    if (previous === undefined) {
      delete process.env.AIANG_RUNTIME_PROFILE
    } else {
      process.env.AIANG_RUNTIME_PROFILE = previous
    }
  })
})
