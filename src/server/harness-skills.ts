...
export function listGlobalSkills(args: { home?: string } = {}): GlobalSkillSummary[] {
  ...
  const addRoot = (dir: string, providers: AgentProvider[]) => {
    for (const skill of scanSkillsRoot(dir)) {
      const existing = byName.get(skill.name)
      if (existing) {
        for (const provider of providers) existing.providers.add(provider)
  ...
  for (const root of globalSkillRoots(home)) addRoot(root.dir, root.providers)
