...
describe("QuickResponseAdapter", () => {
  test("returns the SDK structured result when configured and it validates", async () => {
    const adapter = new QuickResponseAdapter({
      readLlmProvider: async () => ({
        ...
        enabled: true,
        warning: null,
        filePathDisplay: "~/.aiang/llm-provider.json",
      }),
  ...
  test("uses the Kanna app data root as the quick-response workspace", async () => {
    const previousProfile = process.env.AIANG_RUNTIME_PROFILE
    ...
    try {
      ...
      const adapter = new QuickResponseAdapter({
        readLlmProvider: async () => ({
          ...
          enabled: false,
          warning: null,
          filePathDisplay: "~/.aiang-dev/llm-provider.json",
        }),
      ...
      expect(claudeCwd.endsWith("/.aiang-dev")).toBe(true)
    } finally {
      if (previousProfile === undefined) {
        delete process.env.AIANG_RUNTIME_PROFILE
      } else {
        process.env.AIANG_RUNTIME_PROFILE = previousProfile
      }
    }
  })
