...
import { syncCcbSkillsFromAgents } from "./skills"
...
export async function startKannaServer(options: StartKannaServerOptions = {}) {
  ...
  // 启动兜底：把设置页装的技能同步进 ccb（DeepSeek 通道）的 skills 目录，
  // 旧版本安装的技能无需重装即可出现在对话 "/" 菜单。
  syncCcbSkillsFromAgents()
    ...
    try {
      server = Bun.serve<ClientState>({
        ...
        async fetch(req, serverInstance) {
          ...
          // 文件面板：目录树 / 写文件 / 编译 / 读文件。写和读共用同一个
          // 路径模式，各自只处理自己的方法、其余放行给下一个 handler。
          const projectTreeResponse = await handleProjectTree(req, url, store)
          if (projectTreeResponse) {
            return projectTreeResponse
          }

          const projectFileWriteResponse = await handleProjectFileWrite(req, url, store)
          if (projectFileWriteResponse) {
          ...
          // 文件读写路径匹配到了，但方法既不是 GET 也不是 PUT（如 DELETE）。
...
/** 文件读写路径匹配到但方法不是 GET/PUT 时，统一回 405。 */
async function handleProjectFileMethodNotAllowed(url: URL): Promise<Response | null> {
  const match = url.pathname.match(/^\/api\/projects\/([^/]+)\/files\/(.+?)\/content$/)
  if (!match) {
    return null
  }
