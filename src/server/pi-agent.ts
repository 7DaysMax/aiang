...
/** 单个回合的硬性时长上限：超时自动中断，防止 bash 命令/模型循环把回合一直卡在 running。 */
export const PI_MAX_TURN_DURATION_MS = 15 * 60_000

/**
 * 追加给 pi 的系统提示：pi 的 bash 工具默认没有超时，模型不传 timeout 时
 * 一个 `npm run dev` 就能把整个回合永久阻塞，必须约束它。
 */
export const PI_BASH_GUARD_INSTRUCTION = `# Command safety

Long-running commands (dev servers, watchers, \`npm run dev\`, \`python -m http.server\`, installs that can stall) MUST pass a \`timeout\` in seconds to the bash tool (at most 120), or run in the background with \`&\` and redirect output to a file, then poll that file. Never start a blocking command without a timeout: the turn can only continue once the command returns.`

/**
 * 回合看门狗：durationMs 后触发 onExpire，返回取消函数。正常结束时调用取消，
 * 超时时触发中断；触发后取消是安全的（幂等）。
 */
export function scheduleTurnWatchdog(options: {
  durationMs: number
  onExpire: () => void
}): () => void {
  let settled = false
...
export class PiAgentManager {
  ...
  async startTurn(args: StartPiTurnArgs): Promise<HarnessTurn> {
    ...
    let cancelWatchdog = scheduleTurnWatchdog({
      ...
      onExpire: () => {
        queue.push({
          ...
          entry: timestamped({
            kind: "assistant_text",
            text: `⏰ 任务已运行 ${Math.round(PI_MAX_TURN_DURATION_MS / 60_000)} 分钟，已自动中断。可以回复「继续」让它接着完成。`,
          }),
        })
    ...
    // Stable ids so streaming partials merge into one UI row (see parseTranscript).
    let streamTextMessageId: string | null = null
    let streamThinkingMessageId: string | null = null
    let streamedTextSnapshot = ""
    let streamedThinkingSnapshot = ""
    ...
    const unsubscribe = session.subscribe((event: AgentSessionEvent) => {
      switch (event.type) {
        case "message_end": {
          ...
          const text = Array.isArray(message.content)
            ? message.content
              ...
              .join("")
            : ""
          // Prefer a final snapshot only when nothing streamed, or when the final
          // text is a superset of what we already pushed (merge replaces).
          if (text.trim() && text !== streamedTextSnapshot) {
            streamTextMessageId ??= `pi-text-${Date.now()}`
            queue.push({
              type: "transcript",
              entry: timestamped({
                kind: "assistant_text",
                messageId: streamTextMessageId,
                text,
              }),
            })
            streamedTextSnapshot = text
          }
          ...
          const thinking = Array.isArray(message.content)
            ? message.content
              .map((block) => {
                const blockRecord = asRecord(block)
                return blockRecord.type === "thinking" && typeof blockRecord.thinking === "string"
                  ? blockRecord.thinking
                  : ""
              })
              .join("")
            : ""
          if (thinking.trim() && thinking !== streamedThinkingSnapshot) {
            streamThinkingMessageId ??= `pi-thinking-${Date.now()}`
            queue.push({
              type: "transcript",
              entry: timestamped({
                kind: "thinking",
                messageId: streamThinkingMessageId,
                text: thinking,
              }),
            })
            streamedThinkingSnapshot = thinking
          }

          // Next assistant message in this turn (after tools) gets fresh ids.
          streamTextMessageId = null
          streamThinkingMessageId = null
          streamedTextSnapshot = ""
          streamedThinkingSnapshot = ""
          return
        }

        case "tool_execution_start": {
          // Tool boundary: later text/thinking are a new assistant reply.
          streamTextMessageId = null
          streamThinkingMessageId = null
          streamedTextSnapshot = ""
          streamedThinkingSnapshot = ""
        ...
        case "message_update": {
          // Token-level stream from pi-ai. Prefer assistantMessageEvent deltas when
          // present; fall back to the partial message content snapshot.
          const ame = asRecord((event as { assistantMessageEvent?: unknown }).assistantMessageEvent)
          const ameType = asString(ame.type)

          if (ameType === "text_delta") {
            const delta = asString(ame.delta) ?? ""
            if (!delta) return
            streamTextMessageId ??= `pi-text-${Date.now()}`
            streamedTextSnapshot += delta
            queue.push({
              type: "transcript",
              entry: timestamped({
                kind: "assistant_text",
          ...
          if (ameType === "thinking_delta") {
            ...
            if (!delta) return
            streamThinkingMessageId ??= `pi-thinking-${Date.now()}`
            streamedThinkingSnapshot += delta
            queue.push({
              type: "transcript",
              entry: timestamped({
                kind: "thinking",
          ...
          // Snapshot fallback (some pi builds only send partial message).
          ...
          const textBlocks = content
            ...
            .join("")
          if (textBlocks && textBlocks !== streamedTextSnapshot) {
            streamTextMessageId ??= `pi-text-${Date.now()}`
            // Emit only the new suffix when the partial is a growing snapshot.
            const delta = textBlocks.startsWith(streamedTextSnapshot)
              ? textBlocks.slice(streamedTextSnapshot.length)
              : textBlocks
            if (delta) {
              queue.push({
                type: "transcript",
            ...
            streamedTextSnapshot = textBlocks
          }
          ...
          const thinkingBlocks = content
            ...
            .join("")
          if (thinkingBlocks && thinkingBlocks !== streamedThinkingSnapshot) {
            streamThinkingMessageId ??= `pi-thinking-${Date.now()}`
            const delta = thinkingBlocks.startsWith(streamedThinkingSnapshot)
              ? thinkingBlocks.slice(streamedThinkingSnapshot.length)
              : thinkingBlocks
            if (delta) {
              queue.push({
                type: "transcript",
            ...
            streamedThinkingSnapshot = thinkingBlocks
          }
          return
        }
