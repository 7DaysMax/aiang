...
afterEach(() => {
  if (originalRuntimeProfile === undefined) {
    delete process.env.AIANG_RUNTIME_PROFILE
  } else {
    process.env.AIANG_RUNTIME_PROFILE = originalRuntimeProfile
  }
...
function createDeps(overrides: Partial<Parameters<typeof runCli>[1]> = {}) {
  ...
  const deps: Parameters<typeof runCli>[1] = {
    ...
    // Hermetic defaults: never touch the real ~/.aiang/cloud.json or probe
...
describe("classifyInstallVersionFailure", () => {
  test("maps version propagation failures to a user-facing retry message", () => {
    expect(classifyInstallVersionFailure('error: No version matching "0.13.3" found for specifier "aiang"')).toEqual({
      ok: false,
...
describe("runCli", () => {
  ...
  test("starts normally when no newer version exists", async () => {
    const { calls, deps } = createDeps()
    process.env.AIANG_RUNTIME_PROFILE = "prod"

    const result = await runCli(["--port", "4000", "--no-open"], deps)

    expect(result.kind).toBe("started")
    expect(calls.fetchLatestVersion).toEqual(["aiang"])
    ...
    expect(calls.log).toContain("[aiang] data dir: ~/.aiang/data")
  })
  ...
  test("logs the dev data dir when the dev runtime profile is active", async () => {
    ...
    expect(calls.log).toContain("[aiang] data dir: ~/.aiang-dev/data")
  })
  ...
  test("fails fast on unsupported Bun versions", async () => {
    ...
    expect(calls.startServer).toEqual([])
    expect(calls.warn).toContain("[aiang] Bun 1.3.5+ is required for the embedded terminal. Current Bun: 1.3.1")
  })
  ...
  test("logs share setup progress from the default tunnel helper", async () => {
    ...
    deps.startShareTunnel = async (_localUrl) => {
      deps.log("[aiang] installing cloudflared binary")
      installLogged = true
      return {
        publicUrl: "https://kanna.trycloudflare.com",
        stop: () => {},
    ...
    expect(calls.log).toContain("[aiang] installing cloudflared binary")
  })
  ...
  test("fails cleanly when share tunnel startup fails", async () => {
    ...
    expect(calls.warn).toContain("[aiang] failed to start Cloudflare share tunnel")
    expect(calls.warn).toContain("[aiang] cloudflared unavailable")
  })
  ...
  test("keeps running when a named tunnel starts without a detected hostname", async () => {
    ...
    expect(calls.warn).toContain("[aiang] named tunnel started but no public hostname was detected")
    expect(calls.warn).toContain("[aiang] use the hostname configured for the provided Cloudflare tunnel token")
    expect(calls.log).toContain("Local URL:")
  ...
  test("returns restarting when a newer version is available", async () => {
    ...
    expect(calls.installVersion).toEqual([{ packageName: "aiang", version: "0.4.0" }])
    expect(calls.startServer).toEqual([])
  })
  ...
  test("falls back to current version when install fails", async () => {
    ...
    expect(result.kind).toBe("started")
    expect(calls.installVersion).toEqual([{ packageName: "aiang", version: "0.4.0" }])
    expect(calls.warn).toContain("[aiang] update failed, continuing current version")
  })
  ...
  test("falls back to current version when the registry check fails", async () => {
    ...
    expect(result.kind).toBe("started")
    expect(calls.installVersion).toEqual([])
    expect(calls.warn).toContain("[aiang] update check failed, continuing current version")
  })
})
...
describe("runCli cloud", () => {
  test("successful pair flows straight into a normal run (machine comes online immediately)", async () => {
    ...
    expect(calls.log.some((line) => line.includes("starting aiang"))).toBe(true)
