...
describe("installNightlyBuild", () => {
  beforeEach(() => {
    process.env.AIANG_ENABLE_NIGHTLY = "true"
  })

  test("refuses to run while the nightly channel is disabled", async () => {
    delete process.env.AIANG_ENABLE_NIGHTLY
    const result = await installNightlyBuild({})
    expect(result.ok).toBe(false)
    expect(result.userMessage).toContain("暂时禁用")
  })
  ...
  test("downloads main, stamps the version, and runs the build/install steps", async () => {
    ...
    expect(probeEnv.AIANG_DISABLE_SELF_UPDATE).toBe("1")
  })
