...
describe("normalizeClaudeStreamMessage", () => {
  ...
  test("normalizes assistant thinking blocks into thinking entries", () => {
    const entries = normalizeClaudeStreamMessage({
      type: "assistant",
      uuid: "msg-1",
      message: {
    ...
    if (entries[0]?.kind !== "thinking") throw new Error("unexpected entry")
    expect(entries[0].text).toBe("Let me reason about this step by step.")
    ...
    const entries = normalizeClaudeStreamMessage({
      ...
      message: {
        content: [
          ...
          { type: "text", text: "No reasoning needed." },
        ],
      },
    })
...
describe("AgentCoordinator codex integration", () => {
  ...
  test("runTurn writes a stall timeout failure when the engine goes silent", async () => {
    process.env.AIANG_TURN_STALL_TIMEOUT_MS = "80"
    try {
      const store = createFakeStore()
      let turnFailedCount = 0
      store.recordTurnFailed = (async () => { turnFailedCount += 1 }) as never
      ...
      await waitFor(() => store.messages.some((entry) => entry.kind === "result" && entry.result === TURN_STALL_TIMEOUT_MESSAGE), 3000)
      expect(turnFailedCount).toBe(1)
      // 回合必须从 activeTurns 移除，UI 不再显示 Running…。
      expect(coordinator.getActiveStatuses().has("chat-1")).toBe(false)
    } finally {
      delete process.env.AIANG_TURN_STALL_TIMEOUT_MS
    }
  })

  test("runClaudeSession writes a stall timeout failure when the engine goes silent", async () => {
    process.env.AIANG_TURN_STALL_TIMEOUT_MS = "80"
    try {
      const store = createFakeStore()
      let turnFailedCount = 0
...
describe("createClaudeHarnessStream deepseek context window", () => {
  test("overrides the ccb-reported 200k with the real 1M DeepSeek window", async () => {
    async function* fakeQ() {
      yield {
        type: "assistant",
        uuid: "assist-1",
    ...
    // ccb 的 /context 也按内置元数据报 200k——同样要被覆盖成 1M。
    const fake = Object.assign(fakeQ(), {
      getContextUsage: async () => ({ totalTokens: 20_000, maxTokens: 200_000, rawMaxTokens: 200_000, percentage: 10 }),
    }) as never

    const entries: TranscriptEntry[] = []
    for await (const event of createClaudeHarnessStream(fake, { provider: "deepseek" })) {
      if (event.type === "transcript" && event.entry?.kind === "context_window_updated") {
        entries.push(event.entry)
      }
    }

    const finalEntry = entries.at(-1)
    expect(finalEntry?.kind).toBe("context_window_updated")
    if (finalEntry?.kind !== "context_window_updated") throw new Error("missing entry")
    expect(finalEntry.usage.maxTokens).toBe(1_000_000)
    // usedTokens 含 cacheRead：50_000 + 40_000(cache) + 500 = 90_500。
    expect(finalEntry.usage.totalProcessedTokens).toBe(90_500)
    // result 快照不再携带与最后一条 per-step 重复的 last* 增量字段，
    // 否则客户端按增量累加时会把最后一步记两次。
    expect(finalEntry.usage.lastUsedTokens).toBeUndefined()
    expect(finalEntry.usage.lastInputTokens).toBeUndefined()
    expect(finalEntry.usage.lastOutputTokens).toBeUndefined()
  })

  test("claude provider still uses the SDK-reported window", async () => {
    async function* fakeQ() {
      yield {
        type: "assistant",
        uuid: "assist-1",
    ...
    const entries: TranscriptEntry[] = []
    for await (const event of createClaudeHarnessStream(fake, { provider: "claude" })) {
      if (event.type === "transcript" && event.entry?.kind === "context_window_updated") {
        entries.push(event.entry)
      }
    }
    ...
    expect(finalEntry?.kind).toBe("context_window_updated")
    if (finalEntry?.kind !== "context_window_updated") throw new Error("missing entry")
    expect(finalEntry.usage.maxTokens).toBe(200_000)
  })
})
