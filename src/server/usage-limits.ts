...
export function normalizeDeepSeekBalance(
  raw: DeepSeekBalanceSnapshot | null,
  now: string,
  source: UsageLimitSource = "on_demand",
): ProviderUsageSnapshot {
  const base: ProviderUsageSnapshot = {
  ...
  if (!raw.available) {
    const reason =
      raw.error === "missing_key"
        ? "未配置 DeepSeek API Key，无法读取账户余额。"
        : raw.error === "unauthorized"
          ? "DeepSeek API Key 无效或已过期，无法读取账户余额。"
  ...
  const snapshot: ProviderUsageSnapshot = {
    ...
    credits: {
      ...
      balance: raw.totalBalance ?? null,
      detail: null,
      recordedAt: raw.fetchedAt || now,
      source,
    },
  }
  snapshot.updatedAt = raw.fetchedAt || now
  return snapshot
}
...
const PROVIDER_ORDER: AgentProvider[] = ["deepseek", "claude", "codex", "cursor", "pi"]
...
export interface UsageLimitsManagerDeps {
  ...
  /** Fetch the DeepSeek account balance, or null when unavailable. */
  fetchDeepSeekBalance?: () => Promise<DeepSeekBalanceSnapshot | null>
  now?: () => Date
}
...
export class UsageLimitsManager {
  ...
  constructor(filePath: string, deps: UsageLimitsManagerDeps = {}) {
    ...
    // deepseek starts "unknown" until first fetch.
    this.snapshots.set("deepseek", {
      provider: "deepseek", status: "unknown", plan: null, windows: [], credits: null, detail: null, updatedAt: null,
    })
  ...
  async initialize() {
    ...
    try {
      ...
      if (text.trim()) {
        ...
        for (const provider of ["deepseek", "claude", "codex"] as const) {
  ...
  private async refreshDeepSeek() {
    if (!this.deps.fetchDeepSeekBalance) return
    try {
      const raw = await this.deps.fetchDeepSeekBalance()
      this.applyRefreshed("deepseek", normalizeDeepSeekBalance(raw, this.nowIso()))
    } catch (error) {
      const detail = error instanceof Error ? error.message : String(error)
  ...
  private async persist() {
    const file: UsageLimitsFile = {
      version: 1,
      providers: {
        deepseek: this.snapshots.get("deepseek"),
