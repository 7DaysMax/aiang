...
/** Windows 上常见编辑器的安装路径（%LOCALAPPDATA% / %ProgramFiles% 等）。 */
const WINDOWS_EDITOR_EXECUTABLES: Record<"vscode" | "cursor" | "windsurf", string[][]> = {
  vscode: [
...
/**
 * 在 Windows 常见安装位置里找编辑器主程序。`code` 这类命令默认不在 PATH
 * （VSCode 安装时没勾选「添加到 PATH」），但 exe 一定在固定安装目录。
 */
export function findWindowsEditorExecutable(
  preset: "vscode" | "cursor" | "windsurf",
  env: NodeJS.ProcessEnv = process.env,
): string | null {
  for (const segments of WINDOWS_EDITOR_EXECUTABLES[preset]) {
    const [envKey, ...rest] = segments
    const root = env[envKey]
    if (!root) continue
...
function resolveEditorExecutable(preset: Exclude<EditorPreset, "custom">, platform: NodeJS.Platform) {
  if (preset === "cursor") {
    ...
    if (platform === "win32") {
      const exe = findWindowsEditorExecutable("cursor")
  ...
  if (preset === "vscode") {
    ...
    if (platform === "win32") {
      const exe = findWindowsEditorExecutable("vscode")
      if (exe) return { command: exe, args: [] }
    }
  }
  if (preset === "windsurf") {
    ...
    if (platform === "win32") {
      const exe = findWindowsEditorExecutable("windsurf")
      if (exe) return { command: exe, args: [] }
    }
  }
  if (preset === "xcode") {
