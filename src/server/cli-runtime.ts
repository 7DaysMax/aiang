...
Usage:
  ${CLI_COMMAND} [options]
  ${CLI_COMMAND} pair          Claim this machine on Youmi Cloud (prints a link + QR) and start
...
export async function runCli(argv: string[], deps: CliRuntimeDeps): Promise<CliRunResult> {
  ...
  if (runOptions.directCloud) {
    if (!identity) {
      deps.warn(`${LOG_PREFIX} --cloud needs a provisioned cloud identity (~/.kanna/cloud.json) — dev-boxes get one from Youmi Cloud`)
      return { kind: "exited", code: 1 }
    }
  ...
  if (existing) {
    ...
    deps.log(`${LOG_PREFIX} ${CLI_COMMAND} is already running at ${existing.localUrl}${hostedUrl ? ` (and ${hostedUrl})` : ""}`)
    if (hostedUrl) {
