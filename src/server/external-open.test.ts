...
import { buildDefaultOpenCommand, buildEditorCommand, buildPreviewCommand, findWindowsEditorExecutable, tokenizeCommandTemplate } from "./external-open"
...
describe("buildEditorCommand", () => {
  ...
  test("throws a clear error when the editor is missing on Windows", () => {
    expect(() =>
      buildEditorCommand({
        localPath: "C:\\Users\\me\\Projects\\app",
        isDirectory: true,
        editor: { preset: "vscode", commandTemplate: "code {path}" },
        platform: "win32",
      })
    ).toThrow("Visual Studio Code is not installed")
  })
...
describe("findWindowsEditorExecutable", () => {
  test("finds VSCode under LOCALAPPDATA", () => {
    const root = mkdtempSync(path.join(tmpdir(), "win-editor-"))
    const codeExe = path.join(root, "Programs", "Microsoft VS Code", "Code.exe")
    mkdirSync(path.dirname(codeExe), { recursive: true })
    writeFileSync(codeExe, "")

    expect(findWindowsEditorExecutable("vscode", { LOCALAPPDATA: root })).toBe(codeExe)
  })

  test("finds Cursor under LOCALAPPDATA and falls through to ProgramFiles", () => {
    const root = mkdtempSync(path.join(tmpdir(), "win-editor-"))
    const cursorExe = path.join(root, "Programs", "cursor", "Cursor.exe")
    mkdirSync(path.dirname(cursorExe), { recursive: true })
    writeFileSync(cursorExe, "")

    expect(findWindowsEditorExecutable("cursor", { LOCALAPPDATA: root })).toBe(cursorExe)
    expect(findWindowsEditorExecutable("cursor", { ProgramFiles: root })).toBeNull()
  })

  test("returns null when nothing is installed", () => {
    expect(findWindowsEditorExecutable("vscode", {})).toBeNull()
    expect(findWindowsEditorExecutable("windsurf", { LOCALAPPDATA: "/nope" })).toBeNull()
  })
})
