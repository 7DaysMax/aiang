...
describe("normalizeDeepSeekBalance", () => {
  test("maps a fetched balance into an ok snapshot with credits", () => {
    const snapshot = normalizeDeepSeekBalance(
      {
        available: true,
        fetchedAt: NOW,
        currency: "CNY",
  ...
  test("missing key comes back unavailable with a reason", () => {
    const snapshot = normalizeDeepSeekBalance(
      { available: false, fetchedAt: NOW, error: "missing_key" },
      NOW,
    )
    expect(snapshot.status).toBe("unavailable")
    expect(snapshot.credits).toBeNull()
    expect(snapshot.detail).toContain("未配置")
  })
})
...
describe("UsageLimitsManager", () => {
  test("refresh applies both providers, emits, and persists", async () => {
    ...
    expect(snapshot.providers.map((p) => p.provider)).toEqual(["deepseek", "claude", "codex", "cursor", "pi"])
    ...
    expect(snapshot.providers[3]?.status).toBe("unavailable")
    expect(snapshot.providers[4]?.status).toBe("not_applicable")
    expect(snapshot.providers[0]?.credits).toMatchObject({ label: "账户余额", balance: "4.55", currency: "CNY" })
