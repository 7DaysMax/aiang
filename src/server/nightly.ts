...
/**
 * 每日构建通道开关（默认禁用）：当前项目是对 jakemor/kanna 的二次开发，
 * nightly 会从原作者仓库拉 main 分支并全局安装，覆盖本地二改。
 * 恢复发布时设 AIANG_ENABLE_NIGHTLY=true，并把 NIGHTLY_REPO 指向自己的仓库。
 */
function nightlyChannelEnabled(): boolean {
  return process.env.AIANG_ENABLE_NIGHTLY === "true"
}
...
export async function installNightlyBuild(deps: NightlyBuildDeps = {}): Promise<NightlyInstallResult> {
  if (!nightlyChannelEnabled()) {
    return failure("每日构建已暂时禁用：该通道会从原作者仓库（jakemor/kanna）构建并覆盖当前二改版本。如需恢复，请配置 AIANG_ENABLE_NIGHTLY=true 并将 NIGHTLY_REPO 指向自己的仓库。")
  }
  const log = deps.log ?? (() => {})
