...
describe("aiang pair", () => {
  ...
  test("pairing over a stale/outdated cloud.json stays silent about it", async () => {
    ...
    deps.readIdentity = async (warnCb) => {
      warnCb?.("cloud.json is missing tunnelToken, tunnelHost — run `aiang pair` again")
      return null
    }
  ...
  test("management actions still surface a broken cloud.json with the log prefix", async () => {
    ...
    expect(calls.warn[0].startsWith("[aiang] ")).toBe(true)
  })
  ...
  test("bare `aiang pair` prints a claim link + QR and waits for the browser half", async () => {
    const { calls, deps } = createHarness()
  ...
  test("bare `aiang pair` reports an expired link with exit 1", async () => {
    const { calls, deps } = createHarness()
    deps.renderQr = () => "[qr]"
    deps.sleep = async () => {}
