/**
 * ~/.aiang/cloud.json — the machine's cloud pairing credentials, written by
 * `aiang pair <code>` and read on every launch for sticky auto-enable.
...
export function normalizeCloudIdentity(
  ...
  if (missing.length > 0) {
    warn(`cloud.json is missing ${missing.join(", ")} — run \`aiang pair\` again`)
    return null
  }
