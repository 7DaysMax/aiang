...
async function runDeviceCodePairing(args: {
  ...
  if (snapshot.status === "expired") {
    warn(`${LOG_PREFIX} that link expired — run \`aiang pair\` again for a fresh one`)
    return 1
  }
...
export async function runPairCommand(args: PairCommandArgs, deps: PairCommandDeps): Promise<number> {
  ...
  switch (args.action) {
    ...
    case "status": {
      ...
      log(`${LOG_PREFIX} cloud ${identity.enabled ? "starts with every `aiang` run (disable with `aiang pair --disable`)" : "is disabled (enable with `aiang pair --enable`)"}`)
      return 0
    }
    ...
    case "enable": {
      ...
      log(`${LOG_PREFIX} cloud ${enabled ? "enabled — run `aiang` to bring this machine online" : "disabled — `aiang` stays local-only"}`)
      return 0
    }
    ...
    case "remove": {
      ...
      const confirmed = await confirm(`Remove this machine (${identity.appOrigin}) from Youmi Cloud?`)
      if (!confirmed) {
      ...
      try {
        ...
        warn(`${LOG_PREFIX} could not remove the machine on Youmi Cloud (${error instanceof Error ? error.message : String(error)}); deleting local credentials anyway`)
      }
