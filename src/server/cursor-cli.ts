 ...
 *   - { type: "thinking", subtype: "delta"|"completed", text? } (reasoning → thinking)
...
export function parseCursorLine(line: string, configuredModel: string): HarnessEvent[] {
  ...
  switch (type) {
    ...
    case "thinking": {
      // Cursor emits reasoning as { type: "thinking", subtype: "delta"|"completed", text? }.
      // Map non-empty deltas onto our transcript `thinking` kind so the UI can show
      // 「思考中…」; empty delta/completed keepers stay silent.
      const text = asString(value.text)
        ?? asString(value.delta)
        ?? extractAssistantText(value.message)
      if (!text.trim()) return []
      return [{ type: "transcript", entry: timestamped({ kind: "thinking", text }) }]
    }

    // "user" (prompt echo) and unknown types are dropped — the user prompt is
    // already recorded by Kanna before the harness runs.
    default:
      return []
  }
}
