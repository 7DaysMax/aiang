...
export function applyClaudeSdkModels(models: readonly ClaudeSdkModelInfo[]) {
  ...
  const nextModels: ProviderModelOption[] = [
    ...
    // DeepSeek V4 条目是 Claude 入口自带的混合模型（官方 SDK 列表里没有），
    // 每次 SDK 刷新后都保留在末尾。
    ...staticModels.filter((option) => option.id.startsWith("deepseek-")),
  ]
...
export function normalizeClaudeModelOptions(
  ...
  // Claude 入口里的 DeepSeek V4 模型：思考档位只认 low/high/max（V3 的
  // medium 已废弃），无上下文窗口选择器，也不支持 fast mode —— 与 deepseek
  // provider 的归一化规则保持一致。
