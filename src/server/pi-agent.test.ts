...
describe("scheduleTurnWatchdog", () => {
  test("fires once after the duration", async () => {
    let fired = 0
    const cancel = scheduleTurnWatchdog({ durationMs: 20, onExpire: () => { fired += 1 } })
  ...
  test("cancelling before the duration prevents firing", async () => {
    let fired = 0
    const cancel = scheduleTurnWatchdog({ durationMs: 20, onExpire: () => { fired += 1 } })
    cancel()
    await new Promise((resolve) => setTimeout(resolve, 60))
...
describe("PI_BASH_GUARD_INSTRUCTION", () => {
  test("tells the model to always give long-running commands a timeout", () => {
    expect(PI_BASH_GUARD_INSTRUCTION).toContain("timeout")
    expect(PI_BASH_GUARD_INSTRUCTION).toContain("npm run dev")
  })
})
