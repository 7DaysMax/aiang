...
/** Production file-record view backed by the current Git/snapshot diff. */
export function ProjectRecordsTable({ rows, onToggle, onOpen }: { rows: ProjectRecordRow[]; onToggle?: (path: string, selected: boolean) => void; onOpen?: (path: string) => void }) {
  return (
    <div data-beautifului="records-table" className="min-w-[34rem] overflow-hidden rounded-card bg-surface shadow-card">
      <div className="grid grid-cols-[32px_minmax(220px,1fr)_110px_86px_86px] border-b border-[var(--grid-line)] text-[12px] font-medium text-ink-2">
        <span className="px-2 py-2" />
        <span className="border-l border-[var(--grid-line)] px-3 py-2">文件</span>
        <span className="border-l border-[var(--grid-line)] px-3 py-2">类型</span>
        <span className="border-l border-[var(--grid-line)] px-3 py-2 text-right">新增</span>
        <span className="border-l border-[var(--grid-line)] px-3 py-2 text-right">删除</span>
      </div>
      {rows.map((row) => <div key={row.path} className="grid grid-cols-[32px_minmax(220px,1fr)_110px_86px_86px] border-b border-[var(--grid-line)] text-[12.5px] last:border-0 hover:bg-hover">
        <label className="flex items-center justify-center px-2 py-2"><input type="checkbox" checked={row.selected} onChange={(event) => onToggle?.(row.path, event.target.checked)} aria-label={`选择 ${row.path}`} /></label>
        <button type="button" onClick={() => onOpen?.(row.path)} className="truncate border-l border-[var(--grid-line)] px-3 py-2 text-left font-mono text-ink hover:underline">{row.path}</button>
        <span className="border-l border-[var(--grid-line)] px-3 py-2 text-ink-2">{row.changeType}</span>
        <span className="border-l border-[var(--grid-line)] px-3 py-2 text-right font-mono text-green">+{row.additions}</span>
        <span className="border-l border-[var(--grid-line)] px-3 py-2 text-right font-mono text-red">−{row.deletions}</span>
      </div>)}
    </div>
  );
}
