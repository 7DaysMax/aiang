...
export type SearchItem = string | { id: string; label: string; description?: string };
...
export default function SearchList({
  ...
  const itemLabel = (item: SearchItem) => typeof item === "string" ? item : item.label;
  const results = query
    ? items.filter((item) => {
        const haystack = typeof item === "string" ? item : `${item.label} ${item.description ?? ""}`;
  ...
  return (
    <div data-beautifului="search" className={`flex min-h-[248px] w-full max-w-72 flex-col items-stretch ${className}`}>
            ...
            <GlideMenu className="flex flex-col gap-px" highlightClassName="inset-x-0 rounded-[6px] bg-hover">
              {results.map((item) => (
                <button
                  ...
                  data-menu-row
                  type="button"
                  onClick={() => { setQuery(itemLabel(item)); onSelect?.(item); }}
                  className="relative z-10 flex min-h-8 w-full items-center rounded-[6px] px-2 py-1 text-left text-[13px] text-ink"
                  style={{ animation: "fade-in 200ms ease-out both" }}
                >
                  <span className="min-w-0 flex-1 truncate">{itemLabel(item)}</span>
                  {typeof item !== "string" && item.description ? <span className="ml-2 shrink-0 text-[11px] text-ink-3">{item.description}</span> : null}
                </button>
              ))}
            </GlideMenu>
          </div>
