...
import { useEffect, useRef, useState, type CSSProperties, type ReactNode, type ComponentPropsWithoutRef } from "react";
...
/** Production shell: keeps the real sidebar's routing, resizing and data while
 * binding it to the registry component identity and surface contract. */
export function SidebarNavSurface({ children, className, ...props }: ComponentPropsWithoutRef<"div">) {
  return <div data-beautifului="sidebar-nav" className={className} {...props}>{children}</div>;
}
