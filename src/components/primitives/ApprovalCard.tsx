...
export default function ApprovalCard({
  ...
  const items = questions.map((question, index) => ({
    key: question.key ?? String(index),
    q: question.q ?? question.question ?? "",
    type: question.type ?? (question.multiSelect ? "check" as const : "radio" as const),
    options: question.options.map((option) => typeof option === "string" ? { label: option } : option),
  }));
  ...
  const toggle = (index: number) => {
    ...
    if (type === "radio") {
      ...
      advanceTimer.current = setTimeout(() => {
        if (last) send();
        else setQi((current) => Math.min(items.length - 1, current + 1));
      }, 480);
    }
  };
  ...
  return (
    <div data-beautifului="approval-card" className="w-full max-w-80">
      <div className="relative overflow-hidden rounded-card bg-surface shadow-card" style={{ animation: "fade-up 380ms cubic-bezier(0.23,1,0.32,1) both" }}>
        ...
        <div className="primitive-card-footer flex items-center justify-between gap-3">
          ...
          <div className="-mr-0.5 flex items-center gap-1.5">
            <Button variant="ghost" size="sm" onClick={() => { if (last) { setOpen(false); onSkip?.(); } else goTo(qi + 1); }}>
              {t.skip}
            </Button>
