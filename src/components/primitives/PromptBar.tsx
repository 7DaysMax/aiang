...
import type { BeautifulUiPromptBarVariant } from "@/shared/types";
...
/** Controlled production shell for the real Youmi composer. */
export function PromptBarSurface({ children, className = "", optimizing = false, variant = "Rounded" }: { children: ReactNode; className?: string; optimizing?: boolean; variant?: BeautifulUiPromptBarVariant }) {
  return <div data-beautifului="prompt-bar" data-variant={variant} data-optimizing={optimizing ? "true" : undefined} className={`agent-composer-frame ${className}`}>{children}</div>;
}
