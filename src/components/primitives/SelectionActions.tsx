...
import { insertComposerText } from "@/client/lib/composerInsert";
import { CHAT_SELECTION_ZONE_ATTRIBUTE } from "@/client/app/chatFocusPolicy";
...
const LIVE_ACTIONS = [
  { key: "explain", label: "Explain", prefix: "解释这段：\n", icon: <ChatBubbleQuestion {...iconProps} /> },
  { key: "improve", label: "Improve", prefix: "改写得更好：\n", icon: <Spark {...iconProps} /> },
  { key: "shorten", label: "Shorten", prefix: "缩短这段：\n", icon: <Scissor {...iconProps} /> },
] as const;

/** Beautiful UI selection bar connected to real transcript selections. */
export function LiveSelectionActions() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      if (value.length < 2 || !last || !element?.closest(`[${CHAT_SELECTION_ZONE_ATTRIBUTE}]`)) {
        setPicked("");
        setAnchor(null);
        return;
      }
      setPicked(value);
      setAnchor({ x: last.left + last.width / 2, y: last.bottom + 8 });
    };
  ...
  const finish = (text: string) => {
    insertComposerText(text, { replace: true });
  ...
  return (
    <div
      data-beautifului="selection-actions"
      className="pointer-events-auto fixed z-50 flex max-w-[calc(100vw-1rem)] items-center gap-0.5 overflow-x-auto rounded-full bg-surface p-1 shadow-overlay"
      style={{ left: anchor.x, top: anchor.y, transform: "translateX(-50%)", animation: "pop-in 180ms cubic-bezier(0.23,1,0.32,1) both" }}
    >
      {LIVE_ACTIONS.map((item) => (
        <Button key={item.key} variant="quiet" size="xs" onMouseDown={(event) => event.preventDefault()} onClick={() => finish(`${item.prefix}${quoted}`)}>
          {item.icon}{item.label}
        </Button>
      ))}
      <Button variant="primary" size="xs" onMouseDown={(event) => event.preventDefault()} onClick={() => finish(picked)}>
        Send<ArrowUp width="15" height="15" strokeWidth="2.2" aria-hidden />
      </Button>
    </div>
  );
}
