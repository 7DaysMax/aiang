...
export default function ThinkingState({
  ...
  const sequenceStage = useSequence(STAGES);
  const stage = workingProp === undefined ? sequenceStage : workingProp ? 2 : 4;
  const [manualExpanded, setManualExpanded] = useState<boolean | null>(null);
  const [selectedTool, setSelectedTool] = useState<string | null>(null);
  const v = VARIANTS[variant] ?? VARIANTS.Steps;
  const autoExpanded = defaultExpanded ?? (workingProp === undefined ? stage >= 1 && stage < 4 : workingProp);
  const expanded = manualExpanded ?? autoExpanded;
  const working = workingProp ?? stage < 3;
  ...
  return (
    <div
      ...
      key={variant}
      className={`flex w-full flex-col${compact ? "" : " max-w-95"}${className ? ` ${className}` : ""}`}
      style={{
